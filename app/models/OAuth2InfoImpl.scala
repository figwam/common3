 ...
 */
class OAuth2InfoImpl @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def oAuth2InfoSubQuery(loginInfo: LoginInfo) =
    slickOAuth2Infos.filter(_.idLoginInfo in loginInfoQuery(loginInfo).map(_.id))
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val query = for {
      result <- loginInfoQuery(loginInfo).joinLeft(slickOAuth2Infos).on(_.id === _.idLoginInfo)
    } yield result
