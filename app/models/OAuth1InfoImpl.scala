 ...
 */
class OAuth1InfoImpl @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  protected def oAuth1InfoSubQuery(loginInfo: LoginInfo) =
    slickOAuth1Infos.filter(_.idLoginInfo in loginInfoQuery(loginInfo).map(_.id))

  protected def addAction(loginInfo: LoginInfo, authInfo: OAuth1Info) =
    loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      slickOAuth1Infos += DBOAuth1Info(None, authInfo.token, authInfo.secret, dbLoginInfo.id.get, new Timestamp(System.currentTimeMillis))
    }.transactionally
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth1Info): Future[OAuth1Info] = {
    val query = loginInfoQuery(loginInfo).joinLeft(slickOAuth1Infos).on(_.id === _.idLoginInfo)
